...
"""This script is for camera video stream and picture inputs."""
import argparse
import re
import sys
import time
...
def preprocess_image(path, size=(640, 640), layout="nhwc", dtype="uint8"):
    ...
    return preprocess_frame(bgr, size=size, layout=layout, dtype=dtype)


def preprocess_frame(bgr, size=(640, 640), layout="nhwc", dtype="uint8"):
...
def make_text_features(text_runtime, tokenizer, classes):
    input_ids = np.stack([tokenizer.tokenize(text) for text in classes], axis=0)
    outputs = []
    for row in input_ids:
        out = text_runtime.inference(inputs=[row.reshape(1, -1)])
...
def run_clip_text(text_model, classes, vocab_header, target="rk3588", core="auto"):
    ...
    try:
        return make_text_features(rknn, tokenizer, classes)
    finally:
        rknn.release()
...
def draw_detections(image, boxes, scores, class_ids, labels, print_results=True):
    if print_results:
...
def run_yolo_frame(yolo_runtime, frame, text_features, labels, args, fps_text=None):
    img, orig, scale, pad = preprocess_frame(frame, layout=args.layout, dtype=args.input_dtype)
    outputs = yolo_runtime.inference(inputs=[img, text_features])
    ...
    if len(boxes):
        draw_detections(orig, boxes, scores, class_ids, labels, print_results=False)
    if fps_text:
        cv2.putText(orig, fps_text, (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
    return orig, len(boxes)


def update_dynamic_classes(text_runtime, tokenizer, current_classes):
    prompt = "Enter comma-separated classes: "
    if sys.stdin.isatty():
        text = input(prompt).strip()
    else:
        print("Cannot update classes because stdin is not interactive.")
        return current_classes, None
    if not text:
        print("Classes unchanged.")
        return current_classes, None
    classes = parse_class_text(text)
    if not classes:
        print("Classes unchanged.")
        return current_classes, None
    if len(classes) > 80:
        print(f"Too many classes ({len(classes)}), max is 80. Classes unchanged.")
        return current_classes, None
    text_features = make_text_features(text_runtime, tokenizer, classes)
    print("Updated classes:", ", ".join(classes))
...
def parse_args():
    ...
    parser.add_argument("--camera", default=None, help="Camera index or video path for realtime inference")
    ...
    parser.add_argument("--save-video", default=None, help="Optional output video path for camera mode")
    parser.add_argument("--camera-width", type=int, default=0, help="Optional camera capture width")
    parser.add_argument("--camera-height", type=int, default=0, help="Optional camera capture height")
    parser.add_argument("--set-classes-key", default="c", help="Preview key used to update dynamic classes")
    return parser.parse_args()


def main():
    args = parse_args()
    if not args.img and args.camera is None:
        raise ValueError("provide --img for image mode or --camera for realtime mode")
    ...
    if args.text_features:
        ...
        tokenizer = CLIPTokenizer(args.vocab_header)
        text_runtime = create_runtime(args.text_model, target=args.target, core=args.core)
        text_features = make_text_features(text_runtime, tokenizer, labels)
    ...
    try:
        if args.camera is not None:
            ...
            if not cap.isOpened():
                raise RuntimeError(f"failed to open camera/video source: {args.camera}")

            writer = None
            last = time.perf_counter()
            fps = 0.0
            frames = 0
            print("Realtime mode. Press 'q' to quit" + (f", '{args.set_classes_key}' to update classes." if text_runtime else "."))
            while True:
                ok, frame = cap.read()
                if not ok:
                    break
                ...
                last = now
                rendered, det_count = run_yolo_frame(rknn, frame, text_features, labels, args, fps_text=f"FPS {fps:.1f}")
                frames += 1
                ...
                if args.display:
                    cv2.imshow("YOLO-World RKNNLite", rendered)
                    key = cv2.waitKey(1) & 0xFF
                    if key == ord("q"):
                        break
                    if text_runtime and key == ord(args.set_classes_key[:1]):
                        new_labels, new_features = update_dynamic_classes(text_runtime, tokenizer, labels)
                        if new_features is not None:
                            labels, text_features = new_labels, new_features
                elif frames % 30 == 0:
                    print(f"frames={frames} fps={fps:.1f} detections={det_count} classes={','.join(labels)}")
